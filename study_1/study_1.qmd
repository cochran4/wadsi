...
# Clean continuous contrasts using mean +/- SD from predictor_stats_df
...
irr_results$weighted_summary |>
  ...
  dplyr::left_join(
    predictor_stats_df |> dplyr::select(predictor, mean, sd),
    by = "predictor"
  ) |>
  dplyr::left_join(
    binary_predictors,
  ...
  dplyr::ungroup() |>
  dplyr::select(predictor, contrast, posterior_mean, ci_lower, ci_upper) |>
  kable(
