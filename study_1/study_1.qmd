...
We now use the fitted BART model to study what would happen to an individual's risk if we changed one predictor while holding everything else fixed. The model gives, for each individual and each posterior draw $\theta^{(m)}$, a predicted probability of the outcome, $$\widehat{\mu}(x, z; \theta^{(m)}) \approx \Pr(Y = 1 \mid X=x, Z=z, S = 1)$$ Because the model can be evaluated at any input, we can change a predictor $X$ from one value $x'$ to another value $x$, while keeping the covariates Z fixed at their observed values. This lets us compare two hypothetical scenarios for the same individual.

We summarize this comparison using an **individual relative risk:** $$\mathrm{IRR}(x,x',Z) = \frac{\Pr(Y(x)=1 \mid Z)}{\Pr(Y(x')=1 \mid Z)},$$ which measures how much more (or less) likely the outcome would be for a person if their predictor were set to $x$ instead of $x'$. Here, $Y(x)$ and $Y(x')$ denote the outcomes that would be observed for the same individual under these two hypothetical settings of the predictor, with all other variables held fixed.

In practice, we approximate this quantity using the fitted model. For each posterior draw, we evaluate the predicted probabilities under the two scenarios and compute an odds ratio, $$\mathrm{IRR}(x,x',Z) \approx \frac{\widehat{\mu}(x, Z; \theta^{(m)})(1-\widehat{\mu}(x', Z; \theta^{(m)}))}
{\widehat{\mu}(x', Z; \theta^{(m)})(1-\widehat{\mu}(x, z; \theta^{(m)}))}$$ The justification for this approximation, and the assumptions behind it, are given in the protocol.
...
These contrasts produce a posterior distribution of individual relative risks for each person and predictor. We summarize each distribution by its posterior median and visualize the resulting individual-level effects below.
...
-   $\Pr(Y=1)$ and $\Pr(Y=0)$ are specified in the `config` file, reflecting the target population.
...
For each posterior draw, we compute these weights and take a weighted average of the individual relative risks across all individuals. Repeating this across posterior draws gives a distribution for the population relative risk, which we summarize using the posterior median and a 95% credible interval.
...
We next consider individual and population attributable fractions. The individual attributable fraction measures how much of a person’s predicted likelihood of autism is attributable to their predictor profile, relative to a reference profile that represents lower-risk values of the predictors.

This differs from the individual relative risks considered earlier in two ways. First, individual relative risks are defined by comparing two fixed values of a single predictor (e.g., "exposed" vs. "unexposed"), without regard to how common those values are or where individuals lie in the distribution. As a result, they describe the strength of an association for a chosen contrast, but not how much that predictor contributes to the likelihood of autism in practice. Second, they vary one predictor at a time while holding all others fixed. This isolates effects, but does not capture how predictors combine to shape a person's overall risk. The attributable fraction addresses both limitations. It considers all predictors together, reflects how they are distributed in the population, and measures their contribution relative to a common baseline.

The **individual attributable fraction** is defined as $$\mathrm{IAF}(x^*,X,Z) = \frac{\Pr\left( Y=1|X, Z\right) - \Pr\left( Y(x^*)=1|X, Z\right)}{\Pr\left( Y=1|X, Z\right)}.$$ In this case, $X$ represents the full vector of predictors of interest. The numerator is the counterfactual reduction in predicted probability that would occur if the individual's predictors were set to the reference profile $x^*$ instead of their observed values $X$, with $Z$ held fixed. The denominator is the predicted probability under the observed predictors. The attributable fraction therefore measures the proportional reduction in predicted risk under this counterfactual change.
...
Because the attributable fraction can be written as one minus a relative risk comparing the reference and observed predictor profiles, it can be estimated using similar posterior predictions as before. For each posterior draw $\theta^{(m)}$, we compute $$\widehat{\mathrm{IAF}}(x^*,X,Z)=1 - \frac{\widehat{\Pr}(Y=1 \mid X=x^*, Z, S=1; \theta^{(m)})/\widehat{\Pr}(Y=0 \mid X=x^*, Z, S=1; \theta^{(m)})}
{\widehat{\Pr}(Y=1 \mid X, Z, S=1; \theta^{(m)})/\widehat{\Pr}(Y=0 \mid X, Z, S=1; \theta^{(m)})}.$$ Operationally, for each posterior draw and each individual, we use the fitted BART model to generate two predictions: one under the observed predictor profile $X$, and one under the reference profile $x^*$, with covariates $Z$ held fixed. We then form the odds ratio comparing the predicted odds under the reference profile to the predicted odds under the observed profile. Subtracting this quantity from one gives the model-based estimate of the individual attributable fraction. Repeating this calculation across posterior draws yields a posterior distribution for each individual, which we summarize by its median.
...
We plot a histogram of the posterior medians of the individual attributable fractions. Values near zero correspond to individuals whose predictor profiles are close to the low-risk reference. Larger values indicate that a greater share of their predicted risk is tied to their specific predictors, rather than to baseline covariates.

```{r}
# Plot posterior medians of individual AFs
...
In practice, we can estimate the PAF by taking the sample average of individual attributable fractions among autism cases, $$\mathrm{PAF}=\mathbb{E}[\mathrm{IAF}(x^*,X,Z) \mid Y=1, S=1] \approx \mathbb{E}_N\left[ \widehat{\mathrm{IAF}}(x^*,X,Z) \mid Y=1, S=1 \right],$$ which follows, perhaps unexpectedly, from a change-of-variables argument described in the protocol.

```{r}
#| label: tbl-paf
#| tbl-cap: "Posterior median and 95% credible interval for the population attributable fraction"
...
To explain how each predictor contributes to that the attributable fractions, we adapt the idea of Shapley values from cooperative game theory, an approach widely used in explainable AI. In this framework, the individual attributable fraction is expressed as an additive sum of contributions from each predictor, $$\mathrm{IAF}(x^*,X, Z) = \phi_0(x^*,X,Z) + \phi_1(x^*,X, Z) + \phi_p(x^*,X,Z),$$ where $\phi_0(x^*,X,Z)$ is a baseline term and each $\phi_j(x^*,X, Z)$ represents the fair contribution of predictor $j$ to the individual's attributable fraction.

These contributions (called *Shapley values*) are computed by averaging how much each predictor in $X$ contributes to $\mathrm{IAF}(x^*,X, Z)$ when added to subsets of other predictors. These subsets are formed by randomly ordering the predictors and sequentially adding them, evaluating how the attributable fraction shifts at each step. Predictors not yet added are filled in with values drawn from random "donor" individuals, providing realistic replacements that reflect population variability. By averaging these incremental changes over many random orderings, we obtain a fair and order-independent decomposition of $\mathrm{IAF}(x^*,X, Z)$ into Shapley values.
...
Just as the population attributable fraction is defined as the average of individual attributable fractions among autism cases, the population Shapley values are obtained by averaging individual Shapley contributions across those same cases. If $$\mathrm{IAF}(x^*,X, Z) = \phi_0(x^*,X,Z) + \phi_1(x^*,X, Z) + \phi_p(x^*,X,Z),$$ then the population-level decomposition is $$\mathrm{PAF}(x^*) = \bar{\phi}_0(x^*) + \bar{\phi}_1(x^*) + \bar{\phi}_p(x^*),$$ where each $\bar{\phi}_j(x^*)$ is the average contribution of predictor $j$ across all cases. These averaged contributions represent how much each predictor explains, on average, of the total population attributable fraction.
...
af_shapley$population_feature_shapley |>
  dplyr::select(median_shapley, lower, upper, n_cases_used) |>
  dplyr::mutate(
